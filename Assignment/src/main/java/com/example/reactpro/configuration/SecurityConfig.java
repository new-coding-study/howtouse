...
import com.example.reactpro.jwt.JwtAccessDeniedHandler;
import com.example.reactpro.jwt.JwtAuthenticationEntryPoint;
   ...
   // protected void configure(HttpSecurity http) throws Exception {
         ...
         http.csrf().disable()
                ...
                .antMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                 .antMatchers("/api/v1/arts-management").hasRole("ADMIN")  // 나머지 API 는 전부 인증 필요
                 .antMatchers("/api/v1/arts/modern").permitAll()// 제품 누구나 접근가능
                 .antMatchers("/api/v1/arts/contemporary").hasAnyRole("USER", "ADMIN")
                 .antMatchers("/auth/**").permitAll()// 로그인, 회원가입 API 는 토큰이 없는 상태에서 요청이 들어오기 때문에 permitAll 설정
                .antMatchers("/api/v1/art/**").permitAll()// 리뷰도 누구나 접근가능
